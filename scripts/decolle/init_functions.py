...
def init_LSUV_RNN(net, data_batch, tgt_mu=0.0, tgt_var=1.0):
    '''
    Initialization inspired from Mishkin D and Matas J. All you need is a good init. arXiv:1511.06422 [cs],
February 2016.
    '''
    ...
    with torch.no_grad():
        ...
        #def lsuv(net, data_batch):
        for l in net.LIF_layers:
            if hasattr(l.base_layer,'bias_ih'):
                if l.base_layer.bias:
                    l.base_layer.bias_ih.data *= 0
                    l.base_layer.bias_hh.data *= 0
                init.orthogonal_(l.base_layer.weight_ih)
                init.orthogonal_(l.base_layer.weight_hh)
            elif hasattr(l,'rec_layer'):
                if l.rec_layer.bias:
                    l.rec_layer.bias_ih.data *= 0
                    l.rec_layer.bias_hh.data *= 0
                init.orthogonal_(l.rec_layer.weight_ih)
                init.orthogonal_(l.rec_layer.weight_hh)
            else:
                if l.base_layer.bias is not None:
                    l.base_layer.bias.data *= 0
                init.orthogonal_(l.base_layer.weight)
        ...
        while not alldone and cnt<1:
            ...
            for i in range(len(net)):
                ...
                if np.isnan(v) or np.isnan(m):
                    ...
                    raise
                if np.abs(v-tgt_var)>.1:
                    if hasattr(net.LIF_layers[i].base_layer,'weight_ih'):
                        net.LIF_layers[i].base_layer.weight_ih.data /= np.sqrt(np.maximum(v,1e-3))               
                        net.LIF_layers[i].base_layer.weight_ih.data *= np.sqrt(tgt_var)
                        net.LIF_layers[i].base_layer.weight_hh.data /= np.sqrt(np.maximum(v,1e-3))                
                        net.LIF_layers[i].base_layer.weight_hh.data *= np.sqrt(tgt_var)
                    else:
                        net.LIF_layers[i].base_layer.weight.data /= np.sqrt(np.maximum(v,1e-3))                  
                        net.LIF_layers[i].base_layer.weight.data *= np.sqrt(tgt_var)
                    done=False
                ...
                alldone*=done
                print(np.abs(v-tgt_var)> .1, np.abs(m-tgt_mu)> .2, )
                if np.abs(m-tgt_mu)>.2:
                    if hasattr(net.LIF_layers[i].base_layer,'weight_ih'):
                        if net.LIF_layers[i].base_layer.bias:
                            net.LIF_layers[i].base_layer.bias_ih.data -= .5*(m-tgt_mu) 
                            net.LIF_layers[i].base_layer.bias_hh.data -= .5*(m-tgt_mu)
                    else:
                        if net.LIF_layers[i].base_layer.bias is not None:
                            net.LIF_layers[i].base_layer.bias.data -= .5*(m-tgt_mu)
                    done=False
