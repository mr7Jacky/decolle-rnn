...
class BaseLIFLayer(nn.Module):
    ...
    def init_state(self, Sin_t):
        dtype = Sin_t.dtype
        if self.base_layer.__class__.__name__ == "RNNCell":
            device = self.base_layer.weight_hh.device
        else: 
    ...
    def get_device(self):
        if hasattr(self.base_layer, 'hidden_size'): return self.base_layer.weight_hh.device
...
class LIFLayer(BaseLIFLayer):
    ...
    def reset_parameters(self, layer):
        ...
        if hasattr(layer, 'out_channels'):
            ...
            layer.weight_ih.data[:] *= 5e-2
            layer.weight_hh.data[:] *= 5e-2
            if layer.bias:
                layer.bias_ih.data[:] = layer.bias_ih.data[:]*((1-self.alpha)*(1-self.beta))
                layer.bias_hh.data[:] = layer.bias_hh.data[:]*((1-self.alpha)*(1-self.beta))
        else:
...
class DECOLLEBaseRNN(nn.Module):
    requires_init = True
    output_statenames = OrderedDict(zip(['s', 'r', 'u'],[0, 1, 2]))
    def __init__(self):

        self.burnin = 0
        super(DECOLLEBaseRNN, self).__init__()
