...
class LenetDECOLLERNN(DECOLLEBaseRNN):
    def __init__(self,
                 input_shape,
                 Nhid=[1],
                 Mhid=[128],
        ...
        super(LenetDECOLLERNN, self).__init__()
    ...
    def build_mlp_stack(self, Mhid, out_channels): 
        ...
        for i in range(self.num_mlp_layers):
            base_layer = nn.RNNCell(Mhid[i], Mhid[i+1], self.with_bias)
            layer = self.lif_layer_type[i+self.num_conv_layers](base_layer,
                         alpha=self.alpha[i],
                         beta=self.beta[i],
                         alpharp=self.alpharp[i],
    ...
    def build_output_layer(self, Mhid, out_channels):
        if self.with_output_layer:
            i=self.num_mlp_layers
            base_layer = nn.RNNCell(Mhid[i], out_channels)
            layer = self.lif_layer_type[-1](base_layer,
                         alpha=self.alpha[i],
                         beta=self.beta[i],
                         alpharp=self.alpharp[i],
...
class LenetDECOLLERec(DECOLLEBaseRNN):
    def __init__(self,
                 input_shape,
                 Nhid=[1],
                 Mhid=[128],
        ...
        if hasattr(lif_layer_type, '__len__'): 
            self.lif_layer_type = lif_layer_type
            self.lif_rec_layer_type = lif_layer_type
        else:
            self.lif_layer_type = [lif_layer_type]*len(Nhid) + [lif_layer_type]*len(Mhid)
            self.lif_rec_layer_type = [lif_layer_type]*len(Mhid)
